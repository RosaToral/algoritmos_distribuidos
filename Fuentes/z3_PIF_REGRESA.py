...
# PRACTICA 3 Version 2: RECORRIDO DE UN GRAFO A UNA DISTSNCIA
#               PIF Segall
...
'''
  ...
  El algoritmo utiliza un mensaje diferente para los mensajes de ida y de regreso.
  ...
  El nodo i deja de enviar el menssje M cuando ya no tiene mas hijos o cuando el ttl llega a 0
...
class Message():
  '''
    ...
    Con el mensaje M se cuentan los niveles que se han visitado.
  '''
...
class PifSegall(Model):
  ...
  def receive(self, event):
    '''
      ...
      M: mensaje que recorre el nodo de ida. Decrementa el ttl
      REGRESA: mensaje de regreso. Solo hace el recorrido de regreso
    '''
    if event.getName().getText() == "M":
      ...
      if event.getName().getTtl()==0 or self.neighbors==[self.father]:
        ...
        if (False not in self.OK.values() or event.getName().getTtl()==0) and self.father != self.id:
          ...
          newevent = Event(Message("REGRESA", event.getName().getTtl()), self.clock + 1.0, self.father, self.id)
          self.transmit(newevent)
