...
class PifSegall(Model):
  ...
  def init(self):
    ...
    self.contagion = randint(1,len(self.neighbors))
    ...
    if event.getName().getText() == "INICIA":
      ...
      if event.getName().getTtl() > 0:
        ...
        while aux_cont > 0:
          r = choice(self.neighbors)
          flag = 0
          while r==self.father or r in event.getName().getPath() or r in aux:
            r = choice(self.neighbors)
            flag+=1
          if flag == len(self.neighbors):
            break
          else:
            aux.append(r)
      ...
      if len(event.getName().getPath()) > 0 and self.contagion==0:
        p = event.getName().getPath().pop()
        print ("\t Mando de regreso a {} \n".format(p))
        newevent = Event(Message("REGRESA", event.getName().getTtl(), event.getName().getValue(), c, event.getName().getPath()), self.clock + 1.0, p, self.id)
        self.transmit(newevent)
      elif len(event.getName().getPath()) == 0: #la lista ya llego al nodo que inicio la busqueda
